...
async def admin_login(payload: dict):
    ...
    Validates email/password → sends OTP to admin's registered email.
    Does NOT return a token yet — token is issued only after 2FA.
    ...
    try:
        ...
        # TODO Production: Send OTP via email (SendGrid / SMTP)
        # send_admin_2fa_email(user.email, otp_code)

        # DEV: Print to console
        print(f"\n{'='*45}")
        print(f"  [ADMIN 2FA] OTP for {user.email}: {otp_code}")
        print(f"  Expires in: 10 minutes")
        print(f"{'='*45}\n")
        log.info("Admin 2FA OTP generated", email=user.email)
