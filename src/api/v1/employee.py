...
async def get_my_attendance(
    ...
    records = await AttendanceRecord.find(
        AttendanceRecord.attendant_id == PydanticObjectId(str(attendant.id)),
        AttendanceRecord.pump_id == PydanticObjectId(str(attendant.pump_id)),  # double isolation
    ).to_list()
...
async def get_my_leaves(
    ...
    leaves = await LeaveRequest.find(
        ...
        LeaveRequest.pump_id == PydanticObjectId(str(attendant.pump_id)),
...
async def apply_leave(
    ...
    leave = LeaveRequest(
        ...
        pump_id=PydanticObjectId(str(attendant.pump_id)),
...
async def get_my_salary(
    ...
    slips = await SalarySlip.find(
        ...
        SalarySlip.pump_id == PydanticObjectId(str(attendant.pump_id)),
    ).sort("-month").to_list()
